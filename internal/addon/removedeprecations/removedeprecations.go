...
func (h *UpdateRemoveDeprecations) postCodeUpdateHandler(e event.Event) error {
	...
	installed, _ := h.composer.IsPackageInstalled(event.Context(), event.Path(), "palantirnet/drupal-rector")
	if !installed {
		h.logger.Debug("rector is not installed, installing")
		if _, err := h.composer.Require(event.Context(), event.Path(), "palantirnet/drupal-rector"); err != nil {
			return err
		}
	}

	customCodeDirectories, err := h.composer.GetCustomCodeDirectories(event.Context(), event.Path())
	if err != nil {
		return err
	}

	deprecationRemovalResult, err := h.rector.Run(event.Context(), event.Path(), customCodeDirectories)
	if err != nil {
		return err
	}
	...
	if !installed {
		...
		if _, err := h.composer.Remove(event.Context(), event.Path(), "palantirnet/drupal-rector"); err != nil {
			return err
		}
	}
